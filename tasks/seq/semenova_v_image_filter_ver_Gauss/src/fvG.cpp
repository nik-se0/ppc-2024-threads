...
uint8_t get_byte_val(double val, int byte_num) {
...
bool ImageFilterVerGauss::run() {
  ...
  internal_order_test();
    ...
    */
          sum = image[i - 1][j - 1] * kernel[0][0] + image[i - 1][j] * kernel[0][1] +
                image[i - 1][j + 1] * kernel[0][2] + image[i][j - 1] * kernel[1][0] + image[i][j] * kernel[1][1] +
                image[i][j + 1] * kernel[1][2] + image[i + 1][j - 1] * kernel[2][0] + image[i + 1][j] * kernel[2][1] +
